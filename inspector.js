(function(document) {
	...
	function cssPath(el) {
		...
		while ( el ) {
			...
			// Get node's CSS classes (if el.className is a string,) replacing spaces with '.':
            // but, if node is an object, like with SVG files, then do this with the baseVal
			cssClass = ( el.className.baseVal ) ? ( '.' + el.className.baseVal.replace(/\s+/g,'.') ) : (( el.className ) ? ( '.' + el.className.replace(/\s+/g,'.') ) : '');
	...
	function inspectorMouseOver(e) {
		...
		// Set outline:
		element.style.outline = '1px dotted #F06';
        element.style.outlineOffset = '.3rem';
	...
	// ---------------------------------------------
	// custom click performer
	// using array helpers from UnderscoreJS
	function doCustomClickThing(e) {
		var cssArray = cssPath(e.target).split(' ');

		// build array of node classnames, but strip out some Angular stuff
		var newArray = [];
        _.each(cssArray, function(node){
            node = node.split('.');
            node = _.rest(node);
            newArray.push('.' + node.join('.').replace(/\.ng-scope/g,'').replace(/\.ng-isolate-scope/g,'').replace(/\.ng-binding/g,''));
        });

		// only really want the last node's css string
		var cssString = _.last(newArray);

		// print it out in the UI
		$('#css-watcher').html(cssString);
	}
})(document);
